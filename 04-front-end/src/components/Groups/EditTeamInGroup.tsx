...
function EditTeamInGroup() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:10000/api/team/${teamId}`)
      .then(response => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!name) {
      setErrorMessage('Please enter a name for the team.');
      return;
    }
    if (name.length < 4) {
    ...
    axios.put(`http://localhost:10000/api/team/${teamId}`, { name })
      .then(response => {
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
      ...
      <br />
      <button type="submit" className="btn btn-secondary btn-lg">Update a team</button>
      {message && <p>{message}</p>}
    </form>
...
export default EditTeamInGroup;